...
- **`source_uri` column**: Design mentions source URI tracking for pg_mooncake compatibility. Not yet needed.

- **`conninfo` column in sync_groups**: Design mentions remote PG support via conninfo. Not yet needed for single-instance HTAP.

- **~~Standalone binary mode~~**: ~~Design specifies Mode 2 (standalone daemon).~~ Done — `duckpipe-daemon` crate with `duckpipe` binary. libpq FFI eliminated.

---

## Test Coverage

All 19 regression tests pass with DuckDB flush + streaming replication enabled:

| Test | What it covers |
|------|---------------|
| auto_start | Worker auto-start on add_table() |
| api | Group/table management API |
| monitoring | groups()/tables()/status() SRFs |
| streaming | INSERT/UPDATE/DELETE CDC sync |
| snapshot_updates | Initial copy + concurrent updates |
| multiple_tables | Multiple tables in same sync group |
| data_types | Various PostgreSQL data types |
| resync | resync_table() functionality |
| truncate | TRUNCATE propagation |
| stress_append | Many single-row transactions batched |
| toast_unchanged | TOAST unchanged column preservation |
| composite_pk | Composite (multi-column) PK INSERT/UPDATE/DELETE + compaction |
| large_transaction | Single 500-row txn, large batch flush, large UPDATE/DELETE |
| concurrent_tables | Concurrent parallel flush across 5 tables (INSERT/UPDATE/DELETE mixed) |
| multi_group | Multiple independent sync groups, disable/re-enable with WAL catchup |
| error_recovery | Flush error detection (error_message), self-healing after fix |
| rename_table | OID-based WAL routing after ALTER TABLE RENAME, metadata update |
| premature_catchup | CATCHUP→STREAMING transition correctness |
| snapshot_race | Concurrent writes during snapshot |
