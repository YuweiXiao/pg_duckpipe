 ...
 *
 * LogicalRepTupleData in PG 14+ has:
 *   StringInfoData colvalues;  -- single buffer holding all data
 *   char *colstatus;           -- status per column ('n'=null, 'u'=unchanged, 't'=text, 'b'=binary)
 *   const char **coldata;      -- pointers into colvalues buffer
 *   int *collen;               -- length of each column value
 *   int ncols;
 */
static List *
...
decode_message(StringInfo buf, XLogRecPtr lsn, SyncGroup *group, HTAB *batches, HTAB *rel_cache)
{
    ...
    switch (msgtype)
    {
        ...
        case LOGICAL_REP_MSG_INSERT:
        {
            ...
            if (!entry) return; /* Should not happen if protocol flow is correct */
        ...
        case LOGICAL_REP_MSG_UPDATE:
        {
            ...
            /* Treat UPDATE as DELETE + INSERT for simplicity in V1 (easy for column store) */
            /* Or better: Add Update change type. 
               Let's stick to what I defined in header: SYNC_CHANGE_UPDATE. 
               But `batch.c` needs to know how to handle it.
               If I treat it as Delete+Insert, it's safer for now.
            */
