...
typedef struct SyncChange {
    ...
    /* We store stringified values for simplicity in V1 (constructing SQL)
       In a real production version we might store Datums, but that requires more type info.
       For now, let's store the raw LogicalRepTupleData but we MUST deep copy it because
       the buffer is transient.
       Actually, deep copying LogicalRepTupleData is hard.
       Alternative: Convert to SQL strings immediately? No, we want to batch.
       Let's store a custom structure: List of strings (values).
    */
...
/* Worker functions */
extern void ducklake_sync_worker_main(Datum main_arg);
