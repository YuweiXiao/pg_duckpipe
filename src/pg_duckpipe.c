...
_PG_init(void) {
	DefineCustomIntVariable("duckpipe.poll_interval", "Interval in milliseconds between polls", NULL,
	                        &duckpipe_poll_interval, 1000, 100, 3600000, PGC_SIGHUP, GUC_UNIT_MS, NULL, NULL,
	                        NULL);
	...
	DefineCustomBoolVariable("duckpipe.enabled", "Enable pg_duckpipe background worker", NULL,
	                         &duckpipe_enabled, true, PGC_SIGHUP, 0, NULL, NULL, NULL);
}
